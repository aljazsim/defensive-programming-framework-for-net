...
public class FrameworkTest
{
    ...
    {
        var prefixes2 = prefixes.ToDictionary(x => x.Split(new string[] { "=>" }, StringSplitOptions.None)[0], x => x.Split(new string[] { "=>" }, StringSplitOptions.None)[1]);
                                            ...
                                            .OrderBy(x => x.Name))
        {
            ...
            var suffix = method.Name[prefix.Length..];
    ...
    private void TestMethodName(Type type, string methodName)
    {
        ...
        {
            methods.Where(x => x.Name == methodName).ToList().ForEach(this.TestMethodModifiers);
        }
    }
